...
// Import Models (Using ../ to reach root directory)
import Product from '../models/Product.js'; 
import Transaction from '../models/Transaction.js';

// Import Routes (Using ../ to reach root directory)
import orderRoutes from '../routes/orderRoutes.js';
import authRoutes from '../routes/authRoutes.js';

// Import Services (Using ./ to look inside current server/ directory)
...
app.post('/api/paystack/webhook', async (req, res) => {
    ...
    if (event.event === 'charge.success') {
        ...
        try {
            ...
            if (txn) {
                console.log(`Funds HELD for reference: ${reference}`);
                // Trigger email notification
                await sendTransactionEmail(customer.email, "Payment Held in Escrow", `Your payment for reference ${reference} is now held securely.`);
            }
        } catch (error) {
